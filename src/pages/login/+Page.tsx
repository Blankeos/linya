...
export default function LoginPage() {
  ...
  // Surface OAuth callback errors (e.g., ?error=Invalid+OAuth+state)
  onMount(() => {
  ...
  return (
    <div class="dark">
      {/* Animated background gradient — matches Linear */}
      ...
      <div class="relative flex min-h-screen flex-col items-center justify-center px-4">
        <div class="w-[288px] flex flex-col items-center">
          ...
          {/* Top-level error (OAuth errors, etc.) */}
          <Show when={error() && !showEmailForm()}>
            <p class="mt-4 w-full rounded-lg bg-red-500/10 px-3.5 py-2.5 text-center text-[12px] text-red-400">
              {error()}
            </p>
          </Show>
          ...
          <div class="mt-6 flex w-full flex-col gap-3">
            ...
            {/* Email — dark secondary, toggles inline form */}
            <Show
              when={!showEmailForm()}
              fallback={
                <form onSubmit={handleSubmit} class="flex flex-col gap-2">
                  ...
                  <button
                    type="submit"
                    disabled={loading()}
                    class="flex h-11 w-full items-center justify-center rounded-lg bg-[#1c1c1d] px-[18px] text-[13px] font-medium text-[rgb(226,227,229)] transition-colors hover:bg-[#252527] disabled:opacity-50 disabled:cursor-not-allowed"
                    style="box-shadow: rgba(255,255,255,0.133) 0px 0px 0px 0.5px, rgba(0,0,0,0.08) 0px 1px 1px 0px"
                  >
            ...
            {/* Passkey — dark secondary */}
            <button
              type="button"
              onClick={handlePasskeyLogin}
              disabled={passkeyLoading()}
              class="flex h-11 w-full items-center justify-center gap-2.5 rounded-lg bg-[#1c1c1d] px-[18px] text-[13px] font-medium text-[rgb(226,227,229)] transition-colors hover:bg-[#252527] disabled:opacity-50 disabled:cursor-not-allowed"
              style="box-shadow: rgba(255,255,255,0.133) 0px 0px 0px 0.5px, rgba(0,0,0,0.08) 0px 1px 1px 0px"
            >
              <PasskeyIcon class="size-4 shrink-0" />
              {passkeyLoading() ? "Waiting for passkey…" : "Log in with passkey"}
            </button>

            <Show when={passkeyError()}>
              <p class="text-center text-[12px] text-red-400">{passkeyError()}</p>
            </Show>
          </div>
          ...
          <p class="mt-8 text-center text-[13px] font-[450] text-[rgb(147,148,150)]">
            ...
            {" "}or{" "}
            <a
              href={getRoute("/forgot-password/verify")}
              class="text-[rgb(226,227,229)] underline-offset-[2.5px] hover:underline"
            >
              forgot password
            </a>
...
function PasskeyIcon(props: { class?: string }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
